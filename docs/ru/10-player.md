...
Поля, от которых зависит показ рекламы (`iptv`, `torrent_hash`, `youtube`, `continue_play`, `vast_*`), фиксируются при вызове `Player.play()` до события `create`. Их изменение или удаление в обработчике на рекламу не влияет; добавить свой `vast_url` в обработчике можно. `Lampa.Player.play` и сам объект `Lampa.Player` защищены от переопределения — для запуска другого плеера используйте `abort()` или `Player.runas()`.

Рекламные SDK (Google IMA, VASTPlayer) загружаются в изолированный контекст и в `window` страницы не попадают — глобальные `window.google.ima` и `window.VASTPlayer` приложением не используются. Статус CUB Premium для рекламы подтверждается сервером, данные из `localStorage` не учитываются. Скрытие или удаление рекламных элементов (заставки, блока с видео, контейнера баннера) во время показа приводит к остановке воспроизведения с уведомлением «Реклама заблокирована сторонним плагином».
